  ...
  return (
    <motion.nav
      ...
      <div className="flex items-center gap-4">
          ...
          {!isHome && <p className='font-medium'>Dashboard</p>}
          {isHome && (
        ...
        <div className="flex items-center gap-4">
          {!isHome && <><input type="text" className='focus:outline-0 rounded-xl p-0.5 bg-transparent border border-[#343434]' /></>}
        <Link href={isHome ? '/resources':'/'}> <button className={`rounded-md ${isHome ? 'p-2.5 text-md font-semibold':'p-1.5 text-sm font-medium'}  border cursor-pointer hover:bg-[#0251EF] border-[#0251EF] transition duration-300`}>{isHome ?'Lets Start':'Go Back'}</button></Link>
